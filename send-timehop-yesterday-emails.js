...
const handleProcess = async function(user, debugDate) {
    ...
    /* Build email message */
    const subject = i18n.__({phrase: "Your Activity From Yesterday", locale: locale});
    let body = '';

    if (completedAssignmentCount === 1) {
        body += i18n.__({phrase: "Hey %s!<br><p>Yesterday you've completed the \"%s\" assignment.</p>", locale: locale}, user.personalData.givenName, completedAssignments[0].assignment);
    } else if (completedAssignmentCount > 1) {
        body += i18n.__({phrase: "Hey %s!<br><h4>Yesterday you've completed %s assignments.</h4>", locale: locale}, user.personalData.givenName, completedAssignmentCount);
