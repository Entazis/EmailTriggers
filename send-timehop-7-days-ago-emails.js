...
const handleProcess = async function(user, debugDate) {
    ...
    const endDate = new Date(now.getFullYear(), now.getMonth(), now.getDate()-6, 0, 0, 0);
    ...
    /* Build email message */
    const subject = i18n.__({phrase: "Your Activity From a Week Ago", locale: locale});
    let body = '';

    if (completedAssignmentCount === 1) {
        body += i18n.__({phrase: "Hey %s!<br><p>7 days ago you've completed the \"%s\" assignment.</p>", locale: locale}, user.personalData.givenName, completedAssignments[0].assignment);
    } else if (completedAssignmentCount > 1) {
        body += i18n.__({phrase: "Hey %s!<br><h4>7 days ago you've completed %s assignments.</h4>", locale: locale}, user.personalData.givenName, completedAssignmentCount);
